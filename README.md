...
// In Vercel, we MUST have a valid URL (libsql:// or https://), we cannot use a local file
const client = createClient({
  url: dbUrl && dbUrl.length > 0 ? dbUrl : 'libsql://dummy-url-to-prevent-crash.turso.io',
  authToken: dbToken,
});
...
async function initDb() {
  try {
    ...
    try {
      await client.execute(`ALTER TABLE orders ADD COLUMN status TEXT DEFAULT 'pending'`);
    } catch (e) {
...
app.delete('/api/categories/:id', async (req, res) => {
  ...
  try {
    ...
    await client.execute({
      sql: 'DELETE FROM products WHERE categoryId = ?',
      args: [id]
    });
...
app.post('/api/orders', async (req, res) => {
  const { id, customerName, customerSurname, phone, total, date, items, status = 'pending' } = req.body;
  try {
    await client.execute({
      sql: 'INSERT INTO orders (id, customerName, customerSurname, phone, total, date, items, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      args: [id, customerName, customerSurname, phone, total, date, JSON.stringify(items), status]
    });
    res.status(201).json({ success: true });
  } catch (error) {
